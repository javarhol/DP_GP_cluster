...
##############################################################################
#
#    NEW ADDITION: Export GPy Smoothed Means for R/GSEA Preranking (generated by Gemini)
#
##############################################################################

print "\n--- Exporting GPy Smoothed Means for GSEA Preranking ---"

# Define the numeric time points for which you want predictions
# These should match your timepoints. I used 0dpi, 3dpi, 7dpi, 14dpi, 21dpi.
# Use the 't' variable from DP_GP_cluster.py
time_points_numeric = np.array(t).reshape(-1, 1)

# List to store the results
all_cluster_smoothed_means = []

# Iterate through each cluster in optimal_clusters_GP
# Use .iteritems() for Python 2.7 compatibility and efficiency
for cluster_id, dp_cluster in optimal_clusters_GP.iteritems():
    # Check if a GPy model exists for this cluster
    if hasattr(dp_cluster, 'model') and isinstance(dp_cluster.model, GPy.models.GPRegression):
        print "Processing Cluster ID: {}".format(cluster_id) # Python 2.7 print statement with .format()

        # Get the smoothed mean (mu) from the GPy model for the defined time points
        mu, v = dp_cluster.model.predict(time_points_numeric, full_cov=False)

        # mu is typically a 2D array (n_points, 1), so flatten it to 1D
        smoothed_mean_values = mu.flatten()

        # Create a dictionary for this cluster's data
        cluster_row = {
            'ClusterID': cluster_id,
        }
        for i, time_val in enumerate(time_points_numeric.flatten()):
            cluster_row['{}dpi_SmoothedMean'.format(int(time_val))] = smoothed_mean_values[i]

        all_cluster_smoothed_means.append(cluster_row)
    else:
        print "Skipping Cluster ID {}: No GPy GPRegression model found.".format(cluster_id) # Python 2.7 print statement

# Convert the list of dictionaries to a pandas DataFrame
smoothed_means_df = pd.DataFrame(all_cluster_smoothed_means)

# Define the output file path (will be in the same directory as DP_GP_cluster.py or its output_path_prefix)
# Using args.output_path_prefix to maintain consistency with other outputs
output_file = args.output_path_prefix + "_cluster_GPy_smoothed_means.tsv"

# Export to a tab-delimited file
smoothed_means_df.to_csv(output_file, sep='\t', index=False)

print "\nSuccessfully exported GPy smoothed means to {}".format(output_file) # Python 2.7 print statement
print "You can now load this file into R for correlation calculations."
print "\n--- Sample of exported data ---"
print smoothed_means_df.head() # Python 2.7 print statement for pandas DataFrame
